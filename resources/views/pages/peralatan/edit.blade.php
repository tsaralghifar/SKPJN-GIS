                    ...
                    <form action="{{route('peralatan.update', $peralatan->id)}}" method="POST">
                        ...
                        <div class="form-group">
							...
							@foreach($site as $lokasi)
								<option value="{{ $lokasi->id }}" @if($peralatan->id_site == $lokasi->id) selected @endif>{{ $lokasi->nama_proyek }} | {{ $lokasi->koordinat }}</option>
							@endforeach
							</select>
							@error('id_site') 
...
<script>
    ...
    var map = L.map('map', {
    center: [{{ $peralatan->lokasi->koordinat }}],
    zoom: 9,
    layers: [peta1]
    });
    ...
    // get coordinat
    var curLocation = [{{ $peralatan->lokasi->koordinat }}];
